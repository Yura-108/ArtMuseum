...
jest.mock(
  ...
  () =>
    ({ data }: any) => <div>Rendered {data.length} favorite cards</div>,
);
jest.mock(
...
jest.mock(
  ...
  () =>
    ({ fallback, children }: any) => <div>{children || fallback}</div>,
);
...
describe('Favorites', () => {
  ...
  it('renders "no favorites" message when the list is empty', () => {
    ...
    render(<Favorites />);
    expect(
      screen.getByText("You don't have any favorite artworks yet"),
    ).toBeInTheDocument();
  });
